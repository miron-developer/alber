...
func searchUser(userID int, r *http.Request) (interface{}, error) {
	...
	subsCountQ := orm.SQLSelectParams{
		...
		What:    "COUNT(id)",
		Options: orm.DoSQLOption("(receiverUserID = u.id AND value != -1) OR (senderUserID = u.id AND value = 0)", "", ""),
	}
...
func searchGroup(userID int, r *http.Request) (interface{}, error) {
	...
	subsCountQ := orm.SQLSelectParams{
		Table:   "Relations AS r",
		What:    "COUNT(id)",
		Options: orm.DoSQLOption("(receiverGroupID = g.id AND value != -1) OR (senderGroupID = g.id AND value = 0)", "", ""),
	}
	inRlsh := orm.SQLSelectParams{
		Table:   "Relations AS inR",
		What:    "value",
